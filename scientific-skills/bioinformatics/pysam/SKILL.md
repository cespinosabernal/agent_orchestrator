...
- Save processed objects as `.h5ad` (AnnData); use `.zarr` for large datasets that exceed memory
- Figures: PDF (vector) + PNG (300 DPI) saved to `results/figures/`
- Report effect sizes alongside p-values; specify multiple testing correction method (default: Benjamini-Hochberg FDR)
- Include sample sizes (n) for all groups in figure captions or legends