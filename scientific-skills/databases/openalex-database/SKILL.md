...
- Export query results as `.csv` with all metadata columns intact
- Include query timestamp and database version/date accessed in filename or header comment
- Cache raw API responses locally to avoid redundant queries on re-runs
- Log query parameters alongside results for reproducibility