...
- Save trained models with version-stamped filenames (e.g., `model_v1_YYYY-MM-DD.pt` or `.pkl`)
- Log training metrics (loss, validation scores) to CSV at each epoch or evaluation step
- Figures: training curves and evaluation plots as PNG (300 DPI)
- Report evaluation metrics with standard deviation across folds or seeds